...
class HomepagePresenter extends Nette\Application\UI\Presenter
{
    ...
    {
        $this->template->bests = $this->database->table('product')->select('product_id,product_name,platform_name,price')->order('sold')->limit(20);
        $this->template->news = $this->database->table('product')->select('product_id,product_name,platform_name,price,picture,description')->order('uploaded')->limit(8);
